...
**Goal:** Build a Textual-based terminal interface that drives the existing Agent class directly, with dry-run approval, per-tool approval, session persistence, and a Claude Code-like UX.

**Architecture:** A single `tui.py` file containing a Textual `App` subclass that imports the Agent class from `agents.py`, bootstraps Django for ToolLog ORM access, and renders all interactions in the terminal. A `pyproject.toml` provides the `agent` CLI entry point.

**Tech Stack:** Python, Textual (TUI framework, includes Rich), existing LangGraph/LangChain agent from `agents.py`, Django ORM (bootstrapped, not served).

---

### Task 1: Create pyproject.toml with CLI entry point
...
name = "ai-agent"
version = "0.1.0"
description = "AI Agent with TUI and web interfaces"
requires-python = ">=3.8"
dependencies = [
    "textual>=0.50.0",
]
...
Run: `pip install -e .`
Expected: Successfully installed ai-agent, `agent` command available.

**Step 4: Commit**

```bash
git add pyproject.toml
git commit -m "feat: add pyproject.toml with agent CLI entry point"
```

---

### Task 2: Create TUI skeleton — Django bootstrap, Agent init, app launch

**Files:**
- Create: `tui.py`

**Step 1: Create the minimal Textual app with Django bootstrap and Agent creation**
...
Run: `cd C:/Users/Frank/ai_agent && python tui.py`
Expected: Textual app opens in terminal with header, empty chat log, status bar, input box. Press Ctrl+D to exit.

**Step 3: Test `agent` CLI command**
...
```bash
git add tui.py
git commit -m "feat: add TUI with full agent integration, approval flow, sessions"
```

---

### Task 3: Test and fix the /load session number input

The `/load` command shows a numbered list but then sets `_approval_type = "load_session"`. We need to handle the case where the user types a number to select a session.

**Files:**
- Modify: `tui.py` — `_handle_approval_input` method

**Step 1: Update _handle_approval_input to handle session load**

In the `_handle_approval_input` method, add a check before the y/n handling:
...
```bash
git add tui.py
git commit -m "fix: handle session load number selection in approval input"
```

---

### Task 4: Manual integration test — full approval flow

**No code changes — this is a manual test task.**

**Step 1: Test basic conversation**

Run: `python tui.py`
- Type: "What is 2 + 2?"
- Expected: Agent responds with text (status=success), no tool calls.

**Step 2: Test dry-run approval**

- Type: "Read the file requirements.txt"
- Expected: Dry-run plan appears showing `read_file` tool call.
- Type: `y`
- Expected: Tool executes, agent shows file contents.

**Step 3: Test dry-run denial**

- Type: "Delete the file test_delete_me.txt"
- Expected: Dry-run plan appears.
- Type: `n`
- Expected: Plan denied message, agent acknowledges denial.

**Step 4: Test Ctrl+C stop**

- Type a message that triggers tool calls
- Press Ctrl+C during execution
- Expected: Agent stops, header shows "Stopped"

**Step 5: Test slash commands**

- `/help` — shows command list
- `/cwd` — shows working directory
- `/tools` — toggles tools on/off
- `/save My Test` — saves session
- `/sessions` — lists sessions
- `/clear` — clears chat

**Step 6: Test --dir flag**

Run: `agent --dir C:/Users/Frank`
Expected: TUI opens with header showing `C:/Users/Frank` as working directory.
...
Run: `git diff HEAD -- agents.py chat/ settings.py urls.py manage.py requirements.txt utils.py`
Expected: No output (no changes to existing files).

**Step 2: Verify all new files**

Run: `git log --oneline --name-only tui..HEAD`
Expected: Only `tui.py`, `pyproject.toml`, and `docs/plans/` files appear.

**Step 3: Final test run**

Run: `agent`
Expected: TUI launches in current directory, fully functional.

**Step 4: Final commit if needed**

```bash
git add -A
git commit -m "chore: final TUI cleanup"
```
